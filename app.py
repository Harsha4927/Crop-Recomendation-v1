...
model = load_model(os.path.join(BASE_DIR , 'soil_analysis.h5'))
...
names = ["Maize", "Sugarcane", "Cotton", "Tobacco", "Paddy", "Barley", "Cotton", "Wheat", "Millets", "Oil seeds", "Pulses"]
...
def success():
    ...
    if request.method == 'POST':
        if(request.form):
            ...
            if(len(error) == 0):
                return  render_template('success.html' , img  = img , predictions = predictions, random_name=random_name)
            else:
                return render_template('soil.html' , error = error)
...
global loaded_model
loaded_model = pickle.load(open("yield.pkl", "rb"))
# prediction function
